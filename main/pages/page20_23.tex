...
Dostupnosť komerčných dronov a GPU, plus predchádzajúce dataset-y nášho laboratória, robí tieto ciele realistické v priebehu 36 mesiacov.

\subsection*{Kľúčové výstupy hlavného výskumníka (posledných 5 rokov – ilustratívne):}
\begin{itemize}
    \item Prototyp CNN pre detekciu pšeničnej hrdze (TRL 5 $\to$ pokles straty výnosu $\downarrow 18\,\%$).
    \item Spoluautor 2 článkov v rámci programu Horizon EÚ na tému AI v presnom poľnohospodárstve.
    \item Open-source algoritmus na fúziu vlhkosti pôdy (prijatý 3 regionálnymi start-upmi).
\end{itemize}

\subsection*{Kompetencie partnerov:}
\begin{itemize}
    \item Žiadateľská organizácia prináša testovacie polia a tím UAV-pilotov.
    \item Spolupracujúca univerzita prispieva odbornými znalosťami v oblasti hlbokého učenia a diaľkového snímania.
    \item Priemyselný partner (AgroTech Solutions s.r.o.) poskytne poradenstvo pri prenosu na trh.
\end{itemize}

\subsection*{Zapojenie mladých výskumníkov:}
4 PhD študentov (AI \& Robotika, vek $\leq 30$) a 2 MSc študenti v geoinformatike sa zapoja do označovania dát, overovania v teréne a úloh šírenia informácií.
...
\begin{itemize}
    ...
    \item Popularizačné vedecké články a krátke vysvetľujúce videá pre verejnosť.
\end{itemize}


\section{Implementation}

\subsection*{Pracovný plán \& ciele (36 mesiacov):}
\begin{description}
    ...
    \item[P5 (M20-M34):] Pilotné skúšky na 200 ha farmách $\to$ Overený výkon ($\geq 20$\% zisk výnosu oproti kontrolám).
...
Riaditeľská komisia (PI + jeden zástupca každého partnera); mesačné hodnotenie pokroku; monitorovanie rizík pomocou cloudového PM nástroja.
...
45 \% vybavenie (drony, GPU server, senzory), 40 \% personál (AI výskumníci, odborníci na poľnohospodárstvo), 15 \% cestovanie, šírenie a popularizácia – v súlade s uvedenými cieľmi a pokrokom TRL.

\subsection*{Existujúca infraštruktúra}
Žiadateľ vlastní 2 testovacie polia (každé 30 ha), povolenie na lety dronov a základnú flotilu dronov; univerzitný partner prevádzkuje AI výpočtový cluster (200 TFLOPS) a laboratórium na spektrálnu kalibráciu.
%ENGISH
\newpage
...
\\Principal Investigator  & ??? \\ \hline
\\Applicant organisation & ??? \\ \hline
\\Statutory representative(s) of the applicant & ??? \\ \hline
\bottomrule
\end{tabular}
